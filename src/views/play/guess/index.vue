<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <right-toolbar :showSearch.sync="showSearch" @queryTable="listGuess"></right-toolbar>
    </el-row>
...
<script>
import {listGuess, saveGuess, updateGuess, delGuess} from "@/api/play/guess";
  ...
  methods: {
    ...
    listGuess() {
      ...
      listGuess(this.queryParams).then(response => {
        this.GuessList = response.data.list;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateGuess(this.form).then(response => {
            ...
            saveGuess(this.form).then(response => {
              if(response.code === 200) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const GuessIds = row.id || this.ids;

      this.$modal.confirm('是否确认删除游戏横幅编号为"' + GuessIds + '"的数据项？').then(() => {
        this.loading=true;
        // 这里应该调用删除横幅的接口
